            ...
            for extension in &client_hello.extensions {
                match extension {
                    Extension::SupportedEllipticCurves(e) => {
                        ...
                        // MINEDESK PATCH: upstream took elliptic_curves[0]
                        // unconditionally, so a client that lists an
                        // unrecognized/PQ-hybrid group first (e.g. modern
                        // Chrome offering X25519Kyber768/X25519MLKEM768
                        // ahead of classic curves) made the handshake fail
                        // with "invalid named curve" even though the client
                        // also offered a curve this crate does support
                        // further down the same list. Pick the first entry
                        // that actually decodes to a supported curve instead
                        // of blindly trusting position 0. See
                        // https://github.com/webrtc-rs/webrtc/issues/417.
                        state.named_curve = e
                            .elliptic_curves
                            .iter()
                            .copied()
                            .find(|c| *c != NamedCurve::Unsupported)
