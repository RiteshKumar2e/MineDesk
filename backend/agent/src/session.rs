...
/// `backend/src/modules/sessions/routes.ts`'s `grantedCapabilities`
...
pub struct SessionCapabilities {
    ...
    /// `backend/src/vendor/shared/permissions.ts` documents on the TypeScript side.
    pub camera: bool,
    pub microphone: bool,
}
...
/// on `SessionReadyMessage` in `backend/src/vendor/protocol/signaling.ts`.
pub async fn start(
    session_id: String,
...
///
/// See `backend/agent/README.md` for the hardware-encoder upgrade path this is
